...
export const transformToRGS = (config: GameConfig): RGSMathSchema => {
    ...
    const rawType = s.mechanic?.type || 'match_3';
    let mappedType: string = rawType;
    if (rawType === 'match_3') mappedType = 'match_n';
    else if (rawType === 'find_symbol') mappedType = 'symbol_hunt';
    else if (rawType === 'lucky_number') mappedType = 'target_match';
    ...
    } else if (rawType === 'find_symbol') {
        mechanic.grid_size = {
            rows: s.layout?.rows || 3,
            columns: s.layout?.columns || 3
        };
    } else if ((rawType as string) === 'mines') {
...
export interface RGSSymbolConfig {
    id: string;
    category: string;
    image: string;
    meta?: any;
...
export interface RGSConfigSchema {
    gameId: string;
    mechanic: {
        type: string;
        match_count?: number;
    ...
    symbols: RGSSymbolConfig[];
}

export const generateRGSConfig = (config: GameConfig): RGSConfigSchema => {
    const s = config.scratch;
    if (!s) throw new Error("Scratch config missing");

    const rawType = s.mechanic?.type || 'match_3';
    let type: string = rawType;
    let match_count = undefined;
    let variant = 'MATCH3_BASIC'; // Default variant based on spec
    ...
    if (rawType === 'match_3') {
        ...
        type = 'symbol_hunt';
        variant = 'SYMBOL_HUNT_BASIC';
    } else if (rawType === 'lucky_number') {
        type = 'target_match';
        variant = 'TARGET_MATCH_BASIC';
    }

    const symbols: RGSSymbolConfig[] = [];

    const formatImagePath = (imgUrl: string): string => {
        if (!imgUrl) return '';
        if (imgUrl.startsWith('data:image')) return ''; // Should be caught by cleanConfig, if not, don't output raw base64
        if (imgUrl.startsWith('http') || imgUrl.startsWith('/')) return imgUrl; // External or absolute
        if (!imgUrl.startsWith('assets/')) return `assets/${imgUrl}`;
        return imgUrl;
    };

    (s.prizes || []).forEach((p, index) => {
        if (p.id) {
            const rawImage = p.symbolId || p.image || '';
            symbols.push({
                id: p.id,
                category: p.payout > 0 ? 'win' : 'dud',
                image: formatImagePath(rawImage),
                meta: {
                    label: p.name || `Tier ${index}`,
                    payout: p.payout,
                    color: '#FFD700' // Placeholder as per spec (optional)
                }
            });
        }
    });

    if (s.symbols?.loseVariants && s.symbols.loseVariants.length > 0) {
        s.symbols.loseVariants.forEach((imgUrl: string, idx: number) => {
            symbols.push({
                id: `dud_${idx + 1}`,
                category: 'dud',
                image: formatImagePath(imgUrl),
                meta: {
                    label: `Dud ${idx + 1}`
                }
            });
    ...
    return {
        ...
        mechanic: {
            ...
            rows: s.layout?.rows || 3,
            cols: s.layout?.columns || 3,
            variant,
            extra: {} // Can be populated with mechanic-specific extras
        },
        symbols
    };
};
