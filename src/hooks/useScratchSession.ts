...
import { PlatformComms } from '../utils/latam-api/platformComms';
...
export const useScratchSession = (options: ScratchSessionOptions = {}) => {
    ...
    const platformComms = useRef<PlatformComms | null>(null);
    ...
    useEffect(() => {
        ...
        if (session) {
            ...
            console.log("SESSION_ID:", session.sessionId); // User request: log session ID
            setCurrentSessionId(session.sessionId);

            apiClient.current = new LatamApiClient(session);

            // Initialize platform communications for balance syncing
            platformComms.current = new PlatformComms(session.currency, (newBalance) => {
        ...
        return () => {
            if (platformComms.current) {
                platformComms.current.destroy();
            }
        };
    }, []);
        ...
        if (apiClient.current) {
            ...
            try {
                ...
                if (res.code === 0 || res.status === 'Success') {
                    ...
                    // Notify platform to refresh header balance
                    if (platformComms.current) {
                        platformComms.current.requestBalance();
                    }

                    // Generate round asynchronously without blocking UI
                    generateRound().catch(err => console.error('Round generation failed', err));
                    return true;
                } else {
    ...
    const resolveRound = useCallback(async (isRevealed: boolean) => {
        if (isRevealed && gameState !== 'won' && gameState !== 'revealed') {
            if (currentOutcome?.isWin) {
                ...
                if (apiClient.current && currentRoundId.current) {
                    apiClient.current.credit(0, currentRoundId.current, true).then(() => {
                        // Notify platform even on loss to sync any background balance changes
                        if (platformComms.current) {
                            platformComms.current.requestBalance();
                        }
                    }).catch(err => console.error("Close round failed", err));
                }
            }
        }
