// @ts-nocheck
import { ViewContainer, Ticker, Texture, DEG_TO_RAD, Container, Cache, Assets } from 'pixi.js';
import { Vector2, Color, Skeleton, SkeletonClipping, SkeletonData, AnimationState, AnimationStateData, Physics, MeshAttachment, RegionAttachment, ClippingAttachment, SkeletonBounds, AtlasAttachmentLoader, SkeletonBinary, SkeletonJson } from '@esotericsoftware/spine-core';
...
class Spine extends ViewContainer {
    constructor(options) {
        ...
        // FIX: Pass empty options object to ensure ViewContainer initializes correctly
        // avoiding "Cannot read properties of undefined (reading 'autoGarbageCollect')"
        super({});
        // Pixi properties
        ...
        for (let i = 0; i < currentDrawOrder.length; i++) {
            ...
            if (attachment) {
                if (attachment instanceof MeshAttachment || attachment instanceof RegionAttachment) {
                    ...
                    if (attachment instanceof RegionAttachment) {
                        if (attachment.region) {
                            try {
                                ...
                                // Ignore region errors to prevent crash
                            }
                        }
                    } else {
                    ...
                    // DISABLE CLIPPING FOR DEBUGGING
