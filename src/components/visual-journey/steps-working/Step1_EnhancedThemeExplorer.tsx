...
const startRandomSelection = () => {
  ...
  const soundMs = audio.duration * 1000;
  const scale   = soundMs / sumRaw;
  ...
  const handleSelectTheme = (themeId: string) => {
    ...
    // Prepare all of our updates at once with proper structure
    const updates = {
      ...
      theme: {
        ...
        colorScheme: 'warm-vibrant', // Default colorScheme
        mood: 'exciting', // Default mood
        references: theme.keywords || [],
        selectedThemeId: themeId,
        colors: {
          primary: theme.colors?.primary || '#33ccaa',
          secondary: theme.colors?.secondary || '#ff9933',
          accent: theme.colors?.accent || '#ffffff',
          background: theme.colors?.background || '#000000'
        },
        artStyle: 'cartoon', // Default artStyle
        name: theme.name
      }
    };
  ...
  return (
    <div className="absolut inset-0 uw:h-[80vh] p-2 flex flex-col items-center">
      {/* Main container with proper height and centering */}

        {/* Game name input */}
        <div className='flex items-center justify-center relative w-full'>
          <div>
            <div className="relative w-64 h-10 uw:w-[600px] uw:h-64  flex items-center">
              <div className="absolute inset-y-0 left-0 flex items-center pl-2.5">
                <div
                  className="w-4 h-4 uw:w-9 uw:h-9 uw:ml-1 uw:rounded-full  rounded-full flex items-center justify-center text-white text-[10px] uw:text-2xl font-semibold"
              ...
              <input
                ...
                className="w-full pl-8 pr-2 py-1.5 uw:py-8 bg-white border border-gray-200 rounded-lg h-10
                        focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-transparent
                        text-xs uw:text-3xl placeholder-gray-400 shadow-sm uw:pl-16"
              />
              <button
                className={`ml-2 px-3 py-1.5 uw:py-3  min-w-[60px] uw:min-w-[160px] bg-gradient-to-r from-red-600 to-red-700 text-white rounded-lg shadow-sm hover:shadow-md transition-all duration-200 text-xs uw:text-3xl uw:ml-4 font-semibold ${
                ...
                onClick={async () => {
                  ...
                  if (!displayName) {
                    showWarning('Oops!', 'Please enter a valid game name to continue.');
                    return;
                  }

                  const idName = displayName.toLowerCase().replace(/\s+/g, '-');
                  const currentDate = new Date();
                  const formattedDate = currentDate.toISOString().slice(0, 10).replace(/-/g, '');
                  const newGameId = `${idName}_${formattedDate}`;

                  setIsSaving(true);

                  try {
                    // Get the selected theme data
                    const selectedThemeName = selectedThemeObject?.name || 'Default Theme';
                    const selectedThemeId = selectedThemeObject?.id;
                    
                    // Get current gameStore data and ensure it has proper structure
                    const gameStoreData = useGameStore.getState().config;
                    
                    // Ensure the gameStore has the current theme data
                    const updatedGameStoreData = {
                      ...gameStoreData,
                      gameId: newGameId,
                      displayName: displayName,
                      theme: {
                        ...gameStoreData.theme,
                        mainTheme: selectedThemeName,
                        selectedThemeId: selectedThemeId,
                        name: selectedThemeName,
                        colors: selectedThemeObject?.colors || gameStoreData.theme?.colors
                      }
                    };
                    
                    
                    // Save to RGS API with theme data
                    const result = await slotApiClient.saveGameConfig(
                      newGameId, 
                      updatedGameStoreData, 
                      selectedThemeName, 
                      {
                    ...
                    console.error('Error saving to RGS API:', error);
                    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
                    alert(`Failed to save game configuration: ${errorMessage}`);
                  } finally {
                    setIsSaving(false);
                  }
                }}
          ...
          <div className='absolute right-11'>
            <div className="relative w-48 uw:w-[400px] ml-auto">
              <span className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400">
                <Search className="w-4 h-4 uw:w-9 uw:h-9" />
              </span>
              <input
                className='bg-white border border-gray-200 rounded-lg h-10 w-full pl-8
                          focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-transparent
                          text-xs uw:text-3xl placeholder-gray-400 shadow-sm uw:py-8 uw:pl-12'
                type='text'
                placeholder='Search'
                value={search}
      ...
      <div className=" uw:h-full w-full flex flex-col items-center relative">
        {/* Top title and search bar in the same row */}
          <div className=" flex items-center justify-center mt-4 ">
            <h2 className="text-xs uw:text-2xl uw:mb-3 uppercase tracking-widest text-gray-500 font-semibold">Choose your theme</h2>
          </div>
            ...
            {filteredThemes.length === 0
              ? (() => {
                  ...
                  return otherTheme ? (
                    <SwiperSlide
                      ...
                      className="mx-1 my-4 transition-all !w-[190px] !h-[280px] uw:!w-[420px] uw:!h-[610px]"
                    >
                      <motion.div
                        initial={{ opacity: 0.7 }}
              ...
              : filteredThemes.map((theme, idx) => (
                <SwiperSlide
                  key={theme.id}
                  data-theme-id={theme.id}
                  // style={{ width: '420px', height: '610px' }}
                  style={{ width: 'auto', height: 'auto' }}
                  className="mx-1 my-4 transition-all !w-[190px] !h-[280px] uw:!w-[420px] uw:!h-[610px]"
                >
                  <motion.div
        ...
        <div className="flex items-center gap-4 mt-4">
          ...
          <button
            ...
            className="px-5 py-2 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-lg
                    flex items-center gap-1.5 uw:gap-4 shadow-sm hover:shadow-md transform hover:scale-102 
                    transition-all duration-200 font-medium disabled:opacity-70 text-sm uw:text-3xl"
            style={{ background: isSpinning ? `#777777` : `linear-gradient(90deg, ${NINTENDO_RED} 0%, #d40010 100%)` }}
          >
            <Shuffle className="w-4 h-4 uw:w-7 uw:h-7" />
            Random
          </button>
