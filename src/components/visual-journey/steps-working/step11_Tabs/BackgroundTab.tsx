...
import { Play, Upload, ChevronDown, Loader2, Music } from "lucide-react"
...
const BackgroundTab = () => {
    const {
        selectSoundContent,
        setSelectSoundContent,
        individualControl,
        setIndividualControl,
        config, generateAudio
    } = useGameStore();
    const backgroundMusic = config.audio?.backgroundMusic as { MusicBed?: AudioFile | null; AlternateLoop?: AudioFile | null } || {};
    const [prompt, setPrompt] = useState<string>("Western saloon music");
    const [activeQuality, setActiveQuality] = useState<QualityType>("Standard");
    const [selectedSounds, setSelectedSounds] = useState<Set<AudioType>>(new Set(['MusicBed', 'AlternateLoop']));
    const qualities: QualityType[] = ["Lite", "Standard", "AAA"];
    const [isAllGenerating, setIAllGenerating] = useState<boolean>(false);
    // Track loading state for each audio type
    const [isAudioGeneratingMap, setIsAudioGeneratingMap] = useState<{ [key in AudioType]?: boolean }>({});

    const handleGenerateAll = async () => {
        if (!prompt.trim() || isAllGenerating || selectedSounds.size === 0) return;
        setIAllGenerating(true);
        try {
            await Promise.all(
                Array.from(selectedSounds).map(audioType => generateAudio(prompt, audioType))
            );
        } catch (error) {
            console.error('Failed to generate audio:', error);
        } finally {
            setIAllGenerating(false);
        }
    };

    const handleGenerateIndividual = async (audioType: AudioType) => {
        if (!prompt.trim() || isAudioGeneratingMap[audioType]) return;

        setIsAudioGeneratingMap((prev) => ({ ...prev, [audioType]: true }));
        try {
            await generateAudio(prompt, audioType);
        } catch (error) {
            console.error('Failed to generate audio:', error);
        } finally {
            setIsAudioGeneratingMap((prev) => ({ ...prev, [audioType]: false }));
        }
    };
        ...
        <div className="p-4 border rounded-lg space-y-4 w-full max-w-5xl mx-auto bg-white shadow">
            ...
            <div className="flex items-center gap-4 relative">
                <div className="p-2 rounded-full bg-red-100">
                    <Music className="text-red-700 w-5 h-5" />
                </div>
                <div className="flex flex-col">
                    <p className="font-semibold text-lg">Background Music</p>
                    <p className="text-sm text-gray-500">Set the musical foundation for your slot experience</p>
                </div>
            ...
            <div className="flex gap-2 w-full">
                ...
                <button
                    ...
                    disabled={isAllGenerating || !prompt.trim()}
            ...
            <div className="flex gap-4 text-sm text-gray-600">
                ...
                <button className="hover:bg-gray-100 rounded-lg p-1 flex items-center gap-1 "
                    onClick={() => setSelectSoundContent(!selectSoundContent)}>
                    Select Sounds (2) <ChevronDown size={16} />
                </button>
                <button className="hover:bg-gray-100 rounded-lg p-1 flex items-center gap-1 "
            ...
            {selectSoundContent && (
                <div className="p-4 bg-gray-50 border rounded-lg shadow-sm w-full ">
                    ...
                    <div className="flex space-x-2">
                        {(['MusicBed', 'AlternateLoop'] as AudioType[]).map((audioType) => {
                    ...
                    <div className="border p-4 rounded-lg space-y-2">
                        ...
                        <div className="flex gap-2 text-sm">
                            <button
                                ...
                                disabled={isAudioGeneratingMap['MusicBed'] || !prompt.trim()}
                                className="bg-red-500 text-white px-3 py-1 rounded text-sm disabled:opacity-50 flex items-center gap-1"
                            >
                                {isAudioGeneratingMap['MusicBed'] ? <Loader2 size={12} className="animate-spin" /> : null}
                                Generate
                            </button>
                    ...
                    <div className="border p-4 rounded-lg space-y-2">
                        ...
                        <div className="flex gap-2 text-sm">
                            <button
                                ...
                                disabled={isAudioGeneratingMap['AlternateLoop'] || !prompt.trim()}
                                className="bg-red-500 text-white px-3 py-1 rounded text-sm disabled:opacity-50 flex items-center gap-1"
                            >
                                {isAudioGeneratingMap['AlternateLoop'] ? <Loader2 size={12} className="animate-spin" /> : null}
                                Generate
                            </button>
                            <button className="border px-4 py-1 rounded">Upload</button>
                        </div>
