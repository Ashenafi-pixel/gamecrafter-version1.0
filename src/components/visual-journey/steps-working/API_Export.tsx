...
const Step12_APIExport: React.FC = () => {
  ...
  const validateConfig = () => {
    ...
    // Symbol validation
    if (!config.theme?.generated?.symbols || config.theme.generated.symbols.length < 3) {
      errors.push('At least 3 symbols must be generated');
    }
    
    // RTP validation
    if (config.rtp && (config.rtp.targetRTP < 80 || config.rtp.targetRTP > 100)) {
  ...
  return (
    <div className="step-container">
      <h2 className="text-2xl uw:text-3xl font-bold mb-6 uw:mt-3 text-center">Export Game Configuration</h2>
      <p className="text-center uw:text-2xl text-gray-600 mb-6 uw:mt-2 max-w-2xl mx-auto">
      ...
      {errorMessage && (
        <div className="mb-6 p-4 bg-red-700/20 border border-red-800/30 rounded-lg">
          <div className="flex items-start">
            <XCircle className="w-5 h-5 uw:w-8 uw:h-8 text-red-400 mr-2 mt-0.5 flex-shrink-0" />
            <div>
              <h4 className="font-medium uw:text-3xl mb-1 text-red-300">Error</h4>
              <p className="text-sm uw:text-2xl text-red-200">{errorMessage}</p>
              
              {/* Connection troubleshooting */}
              {errorMessage.includes('connect') && (
                <div className="mt-2 text-xs uw:text-4xl bg-red-900/50 p-2 rounded border border-red-800/50">
              ...
              {attemptCount > 0 && (
                <div className="mt-2 text-xs uw:text-2xl text-red-300/80">
      ...
      {lastExportedId && exportStatus === 'success' && (
        <div className="mb-6 p-4 bg-green-800/20 border border-green-800/30 rounded-lg">
          <div className="flex items-start">
            ...
            <div>
              <h4 className="font-medium mb-1 uw:text-3xl text-green-300">Export Successful</h4>
              <p className="text-sm uw:text-2xl text-green-200">
                Game configuration exported successfully with ID: <strong>{lastExportedId}</strong>
              </p>
              <p className="text-xs uw:text-2xl mt-1 text-green-300/70">
                Your game data is now available on the API server and can be accessed using this ID.
              </p>
              
              <div className="mt-3 p-2 bg-green-900/30 rounded border border-green-800/40 text-xs uw:text-xl">
                <div className="flex items-center gap-1 mb-1 text-green-300">
                  <Shield className="w-3.5 h-3.5 uw:w-8 uw:h-8" />
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          ...
          <div
            ...
            >
              <div className="flex items-center">
                <h3 className="text-lg uw:text-3xl font-semibold text-gray-900">Configuration Summary</h3>
              </div>
              <div className="flex gap-2">
            <button
              onClick={() => handleCopy(formattedConfig)}
              className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg uw:text-2xl"
              title="Copy JSON"
            >
              <Copy className="w-5 h-5 uw:w-8 uw:h-8" />
            </button>
            <button
              ...
              title="Download JSON"
            >
              <Download className="w-5 h-5 uw:w-8 uw:h-8" />
            </button>
          </div>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="text-xs uw:text-3xl text-gray-500">Theme</div>
            <div className="font-medium uw:text-2xl text-gray-800">{config.theme?.mainTheme || 'Not set'}</div>
          </div>
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="text-xs uw:text-3xl text-gray-500">Grid Size</div>
            <div className="font-medium uw:text-2xl text-gray-800">
              {config.reels?.layout?.reels || '?'} × {config.reels?.layout?.rows || '?'}
            </div>
          </div>
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="text-xs uw:text-3xl text-gray-500">Pay Mechanism</div>
            <div className="font-medium uw:text-2xl text-gray-800 capitalize">
              {config.reels?.payMechanism === 'betlines' ? 'Paylines' : config.reels?.payMechanism || 'Not set'}
            </div>
          </div>
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="text-xs uw:text-3xl text-gray-500">RTP Target</div>
            <div className="font-medium uw:text-2xl text-gray-800">{config.rtp?.targetRTP || 96}%</div>
          </div>
        </div>
        
        {/* Raw configuration */}
        <div className="mb-4">
          <div className="text-sm uw:text-3xl font-medium text-gray-700 mb-2">Raw Configuration</div>
          <pre className="bg-gray-50 p-4 rounded-lg text-xs uw:text-2xl font-mono overflow-auto max-h-64 uw:max-h-80 border border-gray-200">
            {formattedConfig}
          </pre>
        </div>
        ...
        <div className="mb-4">
          <button
            ...
            className="text-sm uw:text-2xl font-medium text-blue-600 hover:text-blue-800 flex items-center gap-2"
          >
          ...
          {showApiFormat && (
            <div className="mt-2">
              <div className="flex justify-between items-center mb-1">
                <div className="text-sm uw:text-3xl font-medium text-gray-700">API-Ready Format</div>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleCopy(apiReadyConfig)}
                    className="p-1 text-gray-500 hover:text-gray-700 text-xs uw:text-2xl"
                    title="Copy API Format"
                  >
                    <Copy className="w-4 h-4 uw:w-8 uw:h-8" />
                  </button>
                  <button
                  ...
                  >
                    <Download className="w-4 h-4 uw:w-8 uw:h-8" />
                  </button>
                </div>
              </div>
              <pre className="bg-slate-100 p-4 rounded-lg text-xs uw:text-2xl font-mono overflow-auto max-h-64 uw:h-96 border border-slate-200">
                {apiReadyConfig}
              </pre>
            </div>
          )}
        ...
        {validationErrors.length > 0 && (
          <div className="mb-6 p-3  bg-red-50 border border-red-200 rounded-lg">
            ...
            <ul className="list-disc list-inside text-sm uw:text-2xl text-red-600">
        ...
        {testResults && !errorMessage && (
          <div className={`mb-6 p-3 ${testResults.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} border rounded-lg`}>
            <div className={`flex items-center gap-2 ${testResults.success ? 'text-green-600' : 'text-red-600'} uw:text-3xl font-medium mb-1`}>
              {testResults.success ? (
                <Check className="w-5 h-5 uw:w-8 uw:h-8" />
              ) : (
                <AlertTriangle className="w-5 h-5 uw:w-8 uw:h-8" />
              )}
              <span>{testResults.success ? 'Success' : 'Error'}</span>
            </div>
            <p className={`text-sm uw:text-2xl ${testResults.success ? 'text-green-600' : 'text-red-600'}`}>
              {testResults.message}
            </p>
          </div>
        )}
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4">
        <button
          ...
          className="px-6 py-3 uw:text-2xl bg-blue-600 text-white rounded-lg flex items-center justify-center gap-2 hover:bg-blue-700 disabled:bg-blue-300"
        >
        ...
        <button
          ...
          className={`px-6 py-3 uw:text-2xl rounded-lg flex items-center justify-center gap-2 ${
        ...
        >
          {exportStatus === 'loading' ? (
            <>
              <RefreshCw className="w-5 h-5 uw:w-8 uw:h-8 animate-spin" />
              Exporting...
            </>
          ) : exportStatus === 'success' ? (
            <>
            ...
            <>
              <Upload className="w-5 h-5 uw:w-8 uw:h-8" />
              Export to API &amp; RGS
            </>
          )}
        </button>
        ...
        <button
          onClick={handleDownload}
          className="px-6 py-3 uw:text-2xl border border-gray-300 bg-white text-gray-700 rounded-lg flex items-center justify-center gap-2 hover:bg-gray-50"
        >
          <Download className="w-5 h-5 uw:w-8 uw:h-8" />
          Download JSON
        </button>
