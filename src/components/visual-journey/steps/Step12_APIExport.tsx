...
const Step12_APIExport: React.FC = () => {
  ...
  const handleDownloadPlayableZip = async () => {
    ...
    setErrorMessage(null);
    try {
      // Get the apiConfig from the store directly so it's always bundled
      ...
      console.log('📦 Exporting game with API config:', configWithApi.api);
      await downloadGameAsZip({
        gameId: config.gameId || `game_${Date.now()}`,
        gameConfig: configWithApi,
    ...
    <div className="step-container">
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4">
        ...
        <button
          ...
          className={`px-6 py-3 rounded-lg flex items-center justify-center gap-2 ${exportStatus === 'success'
