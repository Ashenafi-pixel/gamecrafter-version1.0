...
const Step5_Production: React.FC = () => {
    ...
    const [aiPrompt, setAiPrompt] = useState(() => {
        ...
        const initialPrompt = config.marketing?.aiPrompt || "";

        // Filter out any base64 image data that might have been accidentally saved

        return initialPrompt.startsWith('data:image/') ? '' : initialPrompt;
    ...
    // Wrapper function to filter out base64 image data from aiPrompt

    const setAiPromptSafe = (value: string) => {

        // Filter out base64 image data

        const filteredValue = value.startsWith('data:image/') ? '' : value;

        setAiPrompt(filteredValue);
    ...
    const generateLandscape = async () => {
        ...
        try {
            const landscapePrompt = `Mobile game landscape background, ${aiPrompt}, casino slot game, 800x600, wide format, vibrant colors, professional game marketing image, no text, high quality`;
            
            console.log('[ProductionFlow] Generating landscape with prompt:', landscapePrompt);
            
            const result = await enhancedOpenaiClient.generateImage(landscapePrompt, {
                quality: 'high',
                size: '1024x1024' // Square that can be cropped to 800x600
            });
            
            if (result.imageUrl) {
                setThumbnailPreview(result.imageUrl);
                console.log('[ProductionFlow] Landscape generated successfully');
            }
        } catch (error) {
            console.error('Landscape generation failed:', error);
            alert('Failed to generate landscape. Please try again.');
        } finally {
    ...
    const generatePoster = async () => {
        ...
        try {
            const posterPrompt = `Mobile game poster, ${aiPrompt}, casino slot game, 300x400, portrait format, vibrant colors, professional game marketing image, no text, high quality`;
            
            console.log('[ProductionFlow] Generating poster with prompt:', posterPrompt);
            
            const result = await enhancedOpenaiClient.generateImage(posterPrompt, {
                quality: 'high',
                size: '1024x1024' // Square that can be cropped to 300x400
            });
            
            if (result.imageUrl) {
                setPosterPreview(result.imageUrl);
                console.log('[ProductionFlow] Poster generated successfully');
            }
        } catch (error) {
        ...
        const timer = setTimeout(() => {
            ...
            updateConfig({
                ...
                marketing: {
                    ...
                    aiPrompt: aiPrompt.trim(), // Only save non-empty, filtered prompts
                                ...
                                <div className="space-y-6 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                                    ...
                                    <div className="bg-gradient-to-br from-indigo-50 to-purple-50 p-6 rounded-xl border border-indigo-100">
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => setAiPromptSafe(e.target.value)}
                                    ...
                                    <div className="flex flex-wrap gap-8 justify-between">
                                        ...
                                        <div className="flex-1 min-w-[300px] flex gap-2">
                                            ...
                                            <button
                                                ...
                                                disabled={!aiPrompt.trim() || isGeneratingLandscape}

                                                className={`flex-1 py-3 bg-indigo-600 text-white rounded-lg text-sm font-bold shadow-md hover:bg-indigo-700 transition-all ${!aiPrompt.trim() || isGeneratingLandscape ? 'opacity-50' : ''}`}

                                            >

                                                {isGeneratingLandscape ? 'Generating...' : 'Generate Landscape'}
                                        ...
                                        <div className="flex-1 min-w-[300px] flex gap-2">
                                            ...
                                            <button
                                                ...
                                                disabled={!aiPrompt.trim() || isGeneratingPoster}

                                                className={`flex-1 py-3 bg-purple-600 text-white rounded-lg text-sm font-bold shadow-md hover:bg-purple-700 transition-all ${!aiPrompt.trim() || isGeneratingPoster ? 'opacity-50' : ''}`}
