...
import { getScratchMathConfig, generateCompleteExport, generateScratchHTML } from '../../../utils/scratch-export-utils';
        ...
        try {
            ...
            const processValue = async (value: any): Promise<any> => {
                if (typeof value === 'string') {
                    // Blob URL (Auto-converted from Uploads)
                    if (value.startsWith('blob:')) {
                        try {
                            ...
                            console.warn("Export Blob Failed", e);
                            return value;
                        }
                    }
            ...
            // [UPDATED] Include marketing assets (thumbnail/poster) in the export
            const fullConfig = {
                scratch: config.scratch || {},
                marketing: config.marketing,
                theme: config.theme // Ensure theme (logo) is included in processing
            };
            ...
            (embeddedConfig.prizes || []).forEach((p: any) => {
                if (p?.id && p?.image) symbolMap[p.id] = p.image;
            });
            const configForHtml = {
                ...embeddedConfig,
                scratch: embeddedConfig,
                theme: {
                    generated: {
                        background: embeddedConfig.layers?.scene?.value || (embeddedConfig as any).background?.image,
                        symbols: symbolMap
                    }
                }
            };
