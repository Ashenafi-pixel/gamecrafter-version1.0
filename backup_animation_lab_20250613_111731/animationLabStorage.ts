...
export async function saveSymbolToAnimationLab(
  ...
  try {
    ...
    // 🚀 DEVELOPMENT MODE: Skip file saving, just store minimal data
    if (process.env.NODE_ENV === 'development' || window.location.hostname === 'localhost') {
      console.log(`[AnimationLab] 🚀 Development mode: Skipping file save, using direct URL`);
      
      // Store minimal sprite data without large images
      const timestamp = Date.now();
      const spriteData: AnimationLabSprite = {
        id: symbolId,
        name: `Symbol ${symbolId}`,
        type: 'original',
        originalImageUrl: imageUrl, // Keep original data URL for preview
        components: [],
        detectionResults,
      ...
      // Only save the minimal registry entry
      try {
        ...
        const minimalRegistry = Object.entries(registry).reduce((acc, [key, value]) => {
          acc[key] = {
            id: value.id,
            name: value.name,
            type: value.type,
            originalImageUrl: value.originalImageUrl.substring(0, 100) + '...', // Truncate URL
            components: value.components.slice(0, 3), // Limit components
            generatedAt: value.generatedAt,
            prompt: value.prompt?.substring(0, 100) // Truncate prompt
          };
          return acc;
        }, {} as any);
        
        localStorage.setItem('animation_lab_registry', JSON.stringify(minimalRegistry));
        console.log(`[AnimationLab] ✅ Minimal data saved to localStorage`);
        
        return imageUrl; // Return original URL for preview
      } catch (storageError) {
        console.warn(`[AnimationLab] ⚠️ Could not save to localStorage:`, storageError);
        return imageUrl; // Still return URL even if storage fails
      }
    }
...
export async function saveSpriteComponentsToAnimationLab(
  ...
  try {
    console.log(`[AnimationLab] 💾 Saving sprite components for ${symbolId}...`);
